...
begin
  ...
  begin
    while (copy(phpcode, startposition, length(const_thestartofprogrammbody)) <>
      const_theendofoperator) and
      (copy(phpcode, startposition, length(const_thestartofprogrammbody)) <>
      const_thestartofprogrammbody) do
      inc(startposition);
    if copy(phpcode, startposition, length(const_thestartofprogrammbody)) = const_thestartofprogrammbody
    then
    begin
...
begin
  ...
  begin
    ...
    begin
      inc(startposition);
      if ansilowercase(copy(phpcode, startposition, length(const_thesession))) = const_thesession
      then
      begin
        j := startposition + length(const_thesession) + const_numberone;
      ...
      if ansilowercase(copy(phpcode, startposition, length(const_theglobal))) = const_theglobal
      then
      begin
        j := startposition + length(const_theglobal) - const_numberone;
...
begin
  ...
  begin
    textfromcode := copy(plaintext, startposition, length(const_theendofline));

    while (textfromcode <> const_theendofline) and
      (startposition <= length(plaintext) - length(const_theendofline) -
      const_numberone) do
    begin
      inc(startposition);
      textfromcode := copy(plaintext, startposition,
        length(const_theendofline));
    end;
    ...
    j := startposition + length(const_theendofline);
    startposition := j;
  end;
...
begin
  ...
  startposition := const_numberone;
  somesymbolsfromcode := copy(phpcode, startposition,
    length(const_phpwithspace));

  while ((copy(phpcode, startposition, length(const_phpwithspace)) <>
    const_phpwithspace) and (startposition <= (length(phpcode) -
    length(const_php)))) and ((somesymbolsfromcode <> const_php) or
    (copy(phpcode, startposition + length(const_php), length(const_phpwithspace)
    ) <> const_theendofline)) do
  begin
    inc(startposition);
    somesymbolsfromcode := copy(phpcode, startposition, length(const_php));
  end;

  if (copy(phpcode, startposition, length(const_phpwithspace))
    = const_phpwithspace) or ((somesymbolsfromcode = const_php) and
    (copy(phpcode, startposition + length(const_php), length(const_theendofline)
    ) = const_theendofline)) then
  begin
    delete(phpcode, const_numberone, startposition - const_numberone);
    startposition := const_numberone;

    while startposition <= (length(phpcode) - length(const_theendofline) -
      const_numberone) do
    begin
      somesymbolsfromcode := copy(phpcode, startposition,
      ...
      j := startposition + length(const_thestartofcomments);

      if ((somesymbolsfromcode = const_thestartofcomments) and
        ((length(phpcode) - const_numberone) >= startposition)) then
      begin

        while ((copy(phpcode, j, length(const_theendofcomments)) <>
          const_theendofcomments) and
    ...
    begin
      ...
      begin
        j := startposition;
        somesymbolsfromcode := copy(phpcode, j, length(const_theendofline));
        ...
        while (copy(phpcode, j, length(const_thespace)) = const_thespace) or
          (copy(phpcode, j - (length(const_theendofline) - const_numberone),
          length(const_theendofline)) <> const_theendofline) do
          dec(j);

        if (copy(phpcode, j - (length(const_theendofline) - const_numberone),
          length(const_theendofline)) = const_theendofline) then
          delete(phpcode, j, startposition - j + (length(const_theendofline) -
            const_numberone));

      end;

      if copy(phpcode, startposition, length(const_thelineofcomments)) <> const_thelineofcomments
      then
        inc(startposition);
    ...
    while startposition <=
      (length(phpcode) - (length(const_theendofline) + const_numberone)) do
    begin
    ...
    begin
      ...
      if ansilowercase(copy(phpcode, startposition, length(const_theglobals))) = const_theglobals
      then
      begin
        delete(phpcode, startposition,
          (length(const_theglobals) + const_numberone));
        flagforconditionschecking := true;
      ...
      begin
        ...
        begin
          ...
          while copy(phpcode, j, length(const_theendofline)) <>
            const_theendofline do
            inc(j);
    ...
    while startposition <= (length(phpcode) - length(const_theendofline)) do
    begin
      skipthesubroutine(phpcode, const_theprocedure, length(const_theprocedure),
        startposition, setofsymbols, indexofsubroutins,
        indexofglobalvariablesinsubroutins);
      skipthesubroutine(phpcode, const_thefunction, length(const_thefunction),
        startposition, setofsymbols, indexofsubroutins,
        indexofglobalvariablesinsubroutins);
      skipthealgorithm(phpcode, const_thefor, length(const_thefor),
        startposition);
      skipthealgorithm(phpcode, const_theif, length(const_theif),
        startposition);
      skipthealgorithm(phpcode, const_theecho, length(const_theecho),
        startposition);
      skipthealgorithm(phpcode, const_theclass, length(const_theclass),
        startposition);
