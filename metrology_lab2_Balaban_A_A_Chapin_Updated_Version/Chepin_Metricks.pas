...
uses
  ...
  Vcl.Controls, Vcl.Forms, Vcl.Dialogs, Vcl.StdCtrls, UnitOfConstantVariables;
...
var
  ...
  Set_Of_Key_Words_For_Variables_Types
    : array [1 .. Const_TheEndOfMasOfKeyWordsForVariables] of ansistring = (
    'int',
    'char',
    'bool',
    'float',
  ...
  Set_Of_Key_Words_For_Input_And_Output
    : array [1 .. Const_TheNumberOfDifferentsTypeOfVariablesUsing]
    of ansistring = (
    'printf',
    'scanf',
    'scanf_s',
...
Function To_Check_The_Belong_To_English_Alphabet(Some_Symbols_From_Code: string;
  I: integer): boolean;
const
...
begin
  Result := false;
  if (((ord(Some_Symbols_From_Code[I]) >=
    Const_TheStartOfLowEnglishAlphabetInASCII) and
    (ord(Some_Symbols_From_Code[I]) <= Const_TheEndOfLowEnglishAlphabetInASCII))
    or (ord(Some_Symbols_From_Code[I]) = Const_TheNumberOfUnderlineInASCII) or
...
begin
  ...
  while I <= length(Variable_That_Need_To_Put) do
  begin
    if To_Check_The_Belong_To_English_Alphabet(Variable_That_Need_To_Put, I)
    then
    else
      delete(Variable_That_Need_To_Put, I, 1);
    inc(I);
  end;
  for I := 1 to Const_TheEndOfMasOfKeyWordsForVariables do
  ...
  if Variable_That_Need_To_Put = Const_TheStar then
    ...
    while (Variable_That_Need_To_Put[length(Const_TheStar)] = Const_TheStar) and
      (Variable_That_Need_To_Put <> Const_TheNullString) do
      delete(Variable_That_Need_To_Put, 1, length(Const_TheStar));
...
begin
  ...
  begin
    ...
    Some_Symbols_From_Code := copy(Line_For_Parasitic_Variables, Start_Position,
      I - Start_Position);
    if Check_The_Line_On_Same_Variables
      (Line_For_Variables_That_For_Input_And_Output, Some_Symbols_From_Code) or
      Check_The_Line_On_Same_Variables(Line_For_Modifiable_Variables,
      Some_Symbols_From_Code) or Check_The_Line_On_Same_Variables
      (Line_For_Control_Variables, Some_Symbols_From_Code) then
    begin

      delete(Line_For_Parasitic_Variables, Start_Position,
        I - Start_Position + 1);
      I := Start_Position;
      dec(I);
    end;
...
begin
  ...
  begin
    ...
    begin
      ...
      WindowsForMetrick.text := WindowsForMetrick.text + Line_From_Code +
        Const_TheEndOfLine + Const_TheNewLine;
  ...
  begin
    ...
    begin
      ...
      if (Some_Symbols_From_Code = TheVariables^.TheVariableInTheList) and
        ((C_Code[J - 1] in Set_Of_Symbols) or
        (copy(C_Code, J - length(Const_TheEndOfLine) - 1,
...
Function To_Check_On_Equally(Start_Position: integer; C_Code: string): boolean;
const
  Const_Equally = '=';
  Const_DoubleEqually = '==';
  Const_NotEqually = '!=';
...
begin
  if ((C_Code[Start_Position] = Const_Equally) and
    ...
    Const_DoubleEqually) and (copy(C_Code, Start_Position - 1,
    length(Const_DoubleEqually)) <> Const_DoubleEqually) and
    (copy(C_Code, Start_Position - 1, length(Const_NotEqually)) <>
    Const_NotEqually)) and (copy(C_Code, Start_Position,
    length(Const_NotEqually)) <> Const_NotEqually)) or
    (copy(C_Code, Start_Position, length(Const_ToIncrease)) = Const_ToIncrease)
    or (copy(C_Code, Start_Position, length(Const_ToDecrease))
    = Const_ToDecrease) then
    Result := true
  else
    Result := false;
end;

Function To_Check_On_Modifable_Variable_End(C_Code: string;
  Start_Position: integer): boolean;
const
  Const_TheSpace = ' ';
  Const_TheSharp = '#';
  Const_TheEndOfLine = '#13#10';
  Const_TheOpenBracket = '(';
begin
  if ((C_Code[Start_Position] <> Const_TheSpace) and
    ...
    length(Const_TheSpace), length(Const_TheEndOfLine)) <> Const_TheEndOfLine))
  then
    Result := true
  else
    Result := false;
...
begin
  ...
  begin
    if To_Check_On_Equally(Start_Position, C_Code) then
    begin
      dec(Start_Position);
      while C_Code[Start_Position] = Const_TheSpace do
        dec(Start_Position);
      J := Start_Position;
      while To_Check_On_Modifable_Variable_End(C_Code, Start_Position) do
        dec(Start_Position);
      inc(Start_Position);
      ...
      Some_Symbols_From_Code := copy(C_Code, Start_Position,
        J - Start_Position + 1);
      I := 1;
      while not To_Check_The_Belong_To_English_Alphabet
        (Some_Symbols_From_Code, I) do
      begin
        delete(Some_Symbols_From_Code, I, 1);
      end;

      for I := 1 to length(Some_Symbols_From_Code) do
...
Function To_Check_On_The_End_Of_Variable(C_Code: string;
  Start_Position: integer): boolean;
const
  Const_TheAmpersand = '&';
  Const_TheComma = ',';
  Const_TheSpace = ' ';
begin
  if (C_Code[Start_Position] = Const_TheSpace) or
    (C_Code[Start_Position] = Const_TheComma) or
    (C_Code[Start_Position] = Const_TheAmpersand) then
    Result := true
  else
    Result := false;
end;
...
procedure To_Find_Variables_That_For_Input_And_Output
  (var Line_For_Variables_That_For_Input_And_Output: string; C_Code: string);
Const
  Const_TheNumberOfDifferentsTypeOfVariablesUsing = 4;
  Const_TheOpenBracket = '(';
  Const_TheEndOfLine = '#13#10';
  ...
  begin
    ...
    begin
      Start_Position := Start_Position +
        length(Set_Of_Key_Words_For_Input_And_Output[I]);
      while To_Check_On_The_End_Of_Variable(C_Code, Start_Position) do
        inc(Start_Position);
      if C_Code[Start_Position] = Const_TheOpenBracket then
      begin
        inc(Start_Position);
...
begin
  ...
  begin
    ...
    begin
      Flag_For_Condition_Checking := false;
      for J := 1 to Const_TheEndOfMasOfKeyWordsForVariables do
    ...
    begin
      ...
      begin
        Some_Symbols_From_Code := copy(C_Code, Start_Position - 1, 1);
        ...
        begin
          while (copy(C_Code, Start_Position - 1, length(Const_TheSpace)) <>
            Const_TheSpace) and
          ...
          Add_To_List_The_Variables_In_Program(C_Code, Start_Position,
            Set_Of_Symbols, copy(C_Code, Start_Position - 1,
            J - Start_Position + 1));
          Start_Position := J - 1;
        end;
        inc(Start_Position);
...
begin
  ...
  while Start_Position <= (length(C_Code) - length(Const_TheEndOfLine) - 1) do
  begin
    Some_Symbols_From_Code := copy(C_Code, Start_Position,
    ...
    begin
      ...
      if (copy(C_Code, J - (length(Const_TheEndOfLine) - 1),
        length(Const_TheEndOfLine)) = Const_TheEndOfLine) then
        delete(C_Code, J, Start_Position - J +
          (length(Const_TheEndOfLine) - 1));
  ...
  begin
    ...
    while ((Some_Symbols_From_Code <> Const_TheStartOfComments) and
      ((length(C_Code) - 1) >= Start_Position)) do
    begin
      inc(Start_Position);
      Some_Symbols_From_Code := copy(C_Code, Start_Position,
        length(Const_TheStartOfComments));
    ...
    if ((Some_Symbols_From_Code = Const_TheStartOfComments) and
      ((length(C_Code) - 1) >= Start_Position)) then
    begin

      while ((copy(C_Code, J, length(Const_TheEndOfComments)) <>
        Const_TheEndOfComments) and ((length(C_Code) - 1) >= J)) do
      begin
        inc(J);
      end;
  ...
  while Start_Position <= (length(C_Code) - (length(Const_TheEndOfLine) + 1)) do
  begin
    Some_Symbols_From_Code := copy(C_Code, Start_Position, 1);

    if Some_Symbols_From_Code = Const_TheDoubleQuote then
    begin
      inc(Start_Position);
      J := Start_Position;
      Some_Symbols_From_Code := copy(C_Code, J, 1);
      ...
      delete(C_Code, Start_Position - 1, J - Start_Position +
        length(Const_TheDoubleQuote));
  ...
  To_Find_Variables_That_For_Input_And_Output
    (Line_For_Variables_That_For_Input_And_Output, C_Code);
  ...
  WindowsForMetrick.text := WindowsForMetrick.text + Const_TheNewLine +
    Const_TheNewLine + 'The variables for input and output' + Const_TheNewLine +
    Line_For_Variables_That_For_Input_And_Output + Const_TheNewLine +
    Const_TheNewLine + 'The modifiable variables' + Const_TheNewLine +
    Line_For_Modifiable_Variables + Const_TheNewLine + Const_TheNewLine +
    'The managers variables' + Const_TheNewLine + Line_For_Control_Variables +
    Const_TheNewLine + Const_TheNewLine + 'The parasitic variables' +
    Const_TheNewLine + Line_For_Parasitic_Variables + Const_TheNewLine +
